...
def click_session_candidate(ctrl: Any) -> None:
    ...
    try:
        src_cn = (getattr(ctrl, "ClassName", "") or "")
        src_name = _normalize_name(getattr(ctrl, "Name", ""))
        src_aid = _normalize_name(getattr(ctrl, "AutomationId", ""))
        dst_cn = (getattr(target, "ClassName", "") or "")
        dst_name = _normalize_name(getattr(target, "Name", ""))
        dst_aid = _normalize_name(getattr(target, "AutomationId", ""))
        print(
            "[WeChat-UIA] click_session_candidate "
            f"src=({src_cn!r},{src_name!r},{src_aid!r}) "
            f"dst=({dst_cn!r},{dst_name!r},{dst_aid!r})"
        )
    except Exception:
        pass
    try:
