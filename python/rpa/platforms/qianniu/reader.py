...
from ...core.rpa_console_log import rpa_heartbeat, rpa_phase
from ...db import (
...
from ...core.incremental import IncrementalDetector, content_hash, make_platform_msg_id
from ...core.layout_parser import LayoutParserConfig, ParsedMessage, parse_chat_layout
from ...core.ocr_engine import BaseOCREngine, PaddleOCREngine, build_ocr_engine
from .chat_parser import (
    parse_qianniu_chat_blocks,
    to_parsed_messages,
)
from .coords import coordinate_space, rect_from_absolute_fields, rect_ratios_to_bitmap_xywh
from .session import (
...
from .layout_config import build_layout_parser_config
...
from ...core.window_lock import hold_platform_window_lock

CONFIG_DIR = Path(__file__).resolve().parents[2] / "config"
DEFAULT_CONFIG = CONFIG_DIR / "qianniu_config.json"
STATE_DIR = Path(__file__).resolve().parents[2] / "_state"
PLATFORM = "qianniu"
...
def _process_current_chat(
    ...
    merge_y_gap: float,
    layout_parser_config: LayoutParserConfig,
    debug_layout: bool,
    scan_idx: int,
...
def run_reader(
    ...
    layout_parser_config = build_layout_parser_config(layout_cfg)
            ...
            if not (list_first_active and len(unread_rows) > 0 and not current_has_unread):
                ...
                if run_chat_ocr:
                    wrote_this_round += _process_current_chat(
                        ...
                        right_threshold=right_threshold,
                        merge_y_gap=merge_y_gap,
                        layout_parser_config=layout_parser_config,
