...
bool QQMessageDao::upsertConversation(int conversationId,
                                      ...
                                      const QJsonObject& payload)
{
    ...
    q.prepare(QStringLiteral(
        ...
        "conversation_id, qq_account_id, qq_conversation_key, display_name, "
        ...
        "qq_account_id = excluded.qq_account_id, "
        "qq_conversation_key = excluded.qq_conversation_key, "
    ...
    if (!q.exec()) {
        qWarning() << "QQMessageDao::upsertConversation failed:" << q.lastError().text();
        return false;
    }
    return true;
}

bool QQMessageDao::createMessageExtension(int messageId,
                                          int conversationId,
                                          const QString& accountId,
                                          const QString& conversationKey,
                                          const QString& displayName,
                                          ...
                                          const QJsonObject& payload)
{
    ...
    q.prepare(QStringLiteral(
        ...
        "message_id, conversation_id, qq_account_id, qq_conversation_key, "
        "qq_display_name, platform_message_id, direction, sender_role, raw_sender, "
        ...
        "qq_account_id = excluded.qq_account_id, "
        "qq_conversation_key = excluded.qq_conversation_key, "
        "qq_display_name = excluded.qq_display_name, "
    ...
    q.bindValue(QStringLiteral(":bubble_rect"),
                meta.value(QStringLiteral("media_rect")).toString(meta.value(QStringLiteral("rect")).toString()));
    ...
    q.bindValue(QStringLiteral(":raw"), jsonCompact(payload));
    if (!q.exec()) {
        qWarning() << "QQMessageDao::createMessageExtension failed:" << q.lastError().text()
                   << "content_image_path=" << payloadString(payload, QStringLiteral("content_image_path"))
                   << "evidence_ref=" << payloadString(payload, QStringLiteral("evidence_ref"));
        return false;
    }
    return true;
}

bool QQMessageDao::deleteForConversation(int conversationId)
{
    if (conversationId <= 0)
        return false;

    QSqlQuery q(Database::getInstance().connection());
    q.prepare(QStringLiteral("DELETE FROM qq_messages WHERE conversation_id = :cid"));
    q.bindValue(QStringLiteral(":cid"), conversationId);
    if (!q.exec()) {
        qWarning() << "QQMessageDao::deleteForConversation messages failed:" << q.lastError().text();
        return false;
    }
    q.prepare(QStringLiteral("DELETE FROM qq_conversations WHERE conversation_id = :cid"));
    q.bindValue(QStringLiteral(":cid"), conversationId);
    if (!q.exec()) {
        qWarning() << "QQMessageDao::deleteForConversation conversations failed:" << q.lastError().text();
        return false;
    }
    return true;
}
